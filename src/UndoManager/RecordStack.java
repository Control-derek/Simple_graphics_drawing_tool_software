...
//            group.getChildren().removeAll(cur);
     ...
     * @param delNd 待删除结点
     */
    public static void nodeDelete(Node delNd) {
        group.getChildren().removeAll(delNd);
        stack.removeIf((Record nrd) -> {
            if(nrd.getNode() == delNd)  // 就是本人！就是本人！
                return true;  // 杀掉！
            Node pre_nrd = nrd.getOldNode();
            while (pre_nrd != null) {
                if(pre_nrd == delNd)  // 祖上是
                    return true;  // 杀！
                pre_nrd = nrd.getOldNode();
            }
            return false;  // 没关系 活！
        });
    }
